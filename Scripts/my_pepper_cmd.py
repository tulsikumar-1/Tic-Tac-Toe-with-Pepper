...
class PepperRobot:
    ...
    def connect(self, pip=os.environ['PEPPER_IP'], pport=9559, alive=True):
    ...
    def tts(self, interaction):
        ...
        self.tts_service.setParameter("outputDevice","192.168.158.37")
    ...
    def my_asay(self, interaction,animation):
        if self.stop_request:
            return
        if self.anspeech_service is None:
            return
        ...
        self.anspeech_service.say("^start("+animation+") " + interaction+" ^wait("+animation+")")
    ...
    def run_behavior(self, bname):
        if self.beh_service!=None:
            try:
                behavior = bname.split('/')[0].split('-')[0]
    ...
    def getBatteryCharge(self):    #no use
        return self.battery_service.getBatteryCharge()


    # Memory

    def memset(self, key, val):#no use
    ...
    def logenable(self,enable=True):    #no use
        if enable:
    ...
    def logdata(self, data): #no use
        if (self.logfile is not None):
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.logfile.write("%s;%r\n" %(timestamp, data))
            self.logfile.flush()

    def logclose(self): #no use
        if (self.logfile != None):
            self.logfile.close()
            self.logfile = None


    def logthread(self): # not used
        t = threading.currentThread()
        while getattr(t, "do_run", True):
            try:
                z = self.getState()
