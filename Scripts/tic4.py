# class for 4x4 tic-tac-toe architecture
...
class Tic:
    ...
    def __init__(self):
        self.board = [
            [Tic.EMPTY, Tic.EMPTY, Tic.EMPTY, Tic.EMPTY],
            [Tic.EMPTY, Tic.EMPTY, Tic.EMPTY, Tic.EMPTY],
            [Tic.EMPTY, Tic.EMPTY, Tic.EMPTY, Tic.EMPTY],
            [Tic.EMPTY, Tic.EMPTY, Tic.EMPTY, Tic.EMPTY]
        ]
    ...
    def move(self, row, col):
        if DEBUG: print(row)
        if DEBUG: print(col)
        if self.board[row][col] != Tic.EMPTY:
            return False
...
def get_game_over_and_winner(board):
    # check rows
    for row in board:
        if row[0] != Tic.EMPTY and row[0] == row[1] == row[2] == row[3]:
            return True, row[0]
    # check columns
    for col in range(4):
        if board[0][col] != Tic.EMPTY and board[0][col] == board[1][col] == board[2][col] == board[3][col]:
            return True, board[0][col]
    # check diagonals
    if board[0][0] != Tic.EMPTY and board[0][0] == board[1][1] == board[2][2] == board[3][3]:
        return True, board[0][0]
    if board[0][3] != Tic.EMPTY and board[0][3] == board[1][2] == board[2][1] == board[3][0]:
...
def get_tic_highlights_for_tablet(board):
    ...
    # check rows
    for row in range(4):
        if board[row][0] != Tic.EMPTY and board[row][0] == board[row][1] == board[row][2] == board[row][3]:
            hl_matrix[row][0] = True
            hl_matrix[row][1] = True
            hl_matrix[row][2] = True
            hl_matrix[row][3] = True
    # check columns
    for col in range(4):
        if board[0][col] != Tic.EMPTY and board[0][col] == board[1][col] == board[2][col] == board[3][col]:
...
# a player threatens a win if there is a row, col or diag where three spaces have its symbol and the third is empty.
# first an aux
def set_is_threatened(tile1, tile2, tile3, tile4, player):
    return (tile1 == tile2 == tile3 == player and tile4 == Tic.EMPTY) or \
           (tile2 == tile3 == tile4 == player and tile1 == Tic.EMPTY) or \
           (tile3 == tile4 == tile1 == player and tile2 == Tic.EMPTY) or \
           (tile4 == tile1 == tile2 == player and tile3 == Tic.EMPTY)

def player_is_threatening(board, player):
    # check rows
    for row in board:
        if set_is_threatened(row[0], row[1], row[2], row[3], player):
            return True
    # check columns
    for col in range(4):
        if set_is_threatened(board[0][col], board[1][col], board[2][col], board[3][col], player):
            return True
    # check diagonals
    if set_is_threatened(board[0][0], board[1][1], board[2][2], board[3][3], player):
        return True
    if set_is_threatened(board[0][3], board[1][2], board[2][1], board[3][0], player):
        return True
